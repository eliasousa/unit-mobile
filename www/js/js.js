    ...
    if (xmlhttp.readyState == 4) {
      ...
      if (xmlhttp.status == 200) {
        ...
        for (var i = 0; i < pastas.length; i++) {
          ...
          h2.setAttribute('style', 'font-size:12px');
          liul.appendChild(h2);
          liul.setAttribute('onclick',
                            'carregarArquivosPasta("' + item.idtProf + '","' 
                                                      + item.nomeProf + '","' 
                                                      + item.pastas + '");');
      ...
      if (xmlhttp.status == 200) {
        ...
        for (var i = 0; i < arquivos.length; i++) {
          ...
          if(arquivo[0].firstChild.nodeValue == "1") {
            ...
            liul.appendChild(h2);
            liul.setAttribute('onclick',
                              'carregarArquivosPasta2("' + idtProf + '","' 
                                                         + item.descricao + '","' 
                                                         + item.chave + '");');
          }else{
            //ARQUIVO
...
function downloadArquivo(idtArquivo, nomeArquivo){
  ...
  var sessionID = localStorage.getItem('sessionID');
  var matricula = localStorage.getItem('matricula');
  var remoteFile = "https://wwws.unit.br/DiscoVirtualNet/Arquivos/DownloadMobileIphone.ashx?idt_arquivo=" + idtArquivo
                   + "&username=" + matricula 
    ...
    fileSystem.root.getFile(localFileName, {create: true, exclusive: false}, function(fileEntry) {
      ...
      fileTransfer.download(
        ...
        function(entry) {
          ...
          window.open(entry.fullPath, '_system', 'location=yes');
        },
        function(error) {
